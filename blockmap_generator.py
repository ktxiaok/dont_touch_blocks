...
class BlockMapGenerator(SingletonEntity, DynamicEntity):
    ...
    # the index means the block position x and the element means (min_y, max_y)
    __player_path: List[Tuple[int, int]]
    ...
    def __generate(self, bmap: BlockMap):
        ...
        for x in range(blockmap.BLOCK_MAP_WIDTH):
            ...
            if player_y_min > 0:
                y_min_exc = random.randint(0, player_y_min)
                color = (
            ...
            if player_y_max < blockmap.BLOCK_MAP_HEIGHT - 1:
                y_max = random.randint(
                    player_y_max + 1, blockmap.BLOCK_MAP_HEIGHT - 1
                )
                color = (
                    random.randint(0, 128), 
                    random.randint(0, 128), 
    ...
    def __get_next_jump_time(self) -> Decimal:
        return Decimal(
            random.uniform(PLAYER_JUMP_INTERVAL_MIN, PLAYER_JUMP_INTERVAL_MAX)
        )
